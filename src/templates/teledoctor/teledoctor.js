...
const TeleDoc = ({intl, pageContext, ...props}) => {
  ...
  const img = props.data.allImg.edges.filter(({ node }) => node.relativePath.includes("/" + id + "."))
  return (
    <Layout>
      <SEO
        ...
        <div className="d-flex justify-content-center">
          <Card style={{ width: '300px', backgroundColor: '#DDE6ED' }}>
            ...
            <Card.Body>
              ...
              <hr />
              <Card.Text className="text-muted">Interested in: {skill.split(",").map(deg=><Badge pill variant="dark" className="shadow mr-1 my-1 px-2 py-1 text-white" style={{fontSize: '10px'}}>{deg}</Badge>)}</Card.Text>
              
              <Card.Text className="text-center">
                <Badge className="mr-2 mb-2">Available on: {consultationTime}</Badge>
                <Badge >Offday: {closeTime}</Badge>
              </Card.Text>
              <Card.Text className="text-center">Consultation Fee: <Badge className="">{fee + ' tk'}</Badge></Card.Text>
              <Card.Text className="text-center">For online consultation, Please call at 16007</Card.Text>
            </Card.Body>
            <Card.Footer>
              <Share />
            </Card.Footer>
